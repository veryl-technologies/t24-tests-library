...
Cash Deposit - Local - Foreign
    ...
    @{validationRules}=    Create List    WORKING.BALANCE :EQ:= ? round (${w_balance} / ${SREUR}, 2)
    Check T24 Record    ACCOUNT    ${m_teller_savings_acc_fcy}    ${validationRules}
...
Sell Foreign Currency Against Local Currency
    ...
    Create Or Amend T24 Record    TELLER,SELL.FCY.LCASH    >>CurSellrFCY    ${testDataFields}    \    ${EMPTY}

Buy FCY agains cash in LCY
    @{testDataFields}=    Create List    CURRENCY.1=GBP    AMOUNT.FCY.1:1=100.50    NARRATIVE.1:1:1=BUY GBP    UNIT:1=1    UNIT:3=6
    ...    UNIT:6=4    UNIT:8=1    UNIT:10=4    UNIT:11=1    DR.UNIT:1=2    DR.UNIT:7=1
    Create Or Amend T24 Record    TELLER,BUYFCY.CASH    >>CurrBuyFCY    ${testDataFields}    \    ${EMPTY}
